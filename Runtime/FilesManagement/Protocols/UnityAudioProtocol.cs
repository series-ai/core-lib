	...
	public class UnityAudioProtocol : IProtocol
	{
		...
		public async Task<object> Read<T>(string uri, CancellationToken cancellationToken, string version = null)
		{
			...
			using (UnityWebRequest wr = new UnityWebRequest(requestUri, "GET", dh, null)) 
			{
				...
				Debug.LogError($"Download failed. Uri {requestUri} Response {wr.responseCode}. Error: {wr.error}", Constants.DEBUG_CHANNEL_FILES);
			}
        
			Debug.LogError("The download process is not completely finished.", Constants.DEBUG_CHANNEL_FILES);
